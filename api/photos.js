...
// Admin sees every photo; customers only see their own.
async function handleList(req, res) {
  ...
  try {
    const photos = user.role === 'admin'
      ? await sql`
          SELECT p.id, p.user_id, p.frame_id, p.image_data, p.printed, p.expires_at, p.created_at,
                 u.username
          FROM photos p
        ...
        `
      : await sql`
          SELECT id, user_id, frame_id, image_data, printed, expires_at, created_at
          FROM photos
